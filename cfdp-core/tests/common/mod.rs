...
pub(crate) struct TestUser {
    ...
    // a mapping of currently running IDs to originating transacion ID on the daemon
            ...
            while let Ok(indication) = indication_rx.recv() {
                ...
                match indication {
                    Indication::MetadataRecv(MetadataRecvIndication {
                        ...
                        for request in put_requests {
                            ...
                            if let Indication::Transaction(id) =
                                recv.recv().expect("Recv channel disconnected: ")
                            {
                                (*proxy_map.write().unwrap()).insert(id, origin_id);
                            };
                            println!("Adone");
                        }
                    ...
                    Indication::Finished(FinishedIndication {
                        ...
                        if let Some(origin) = proxy_map.read().unwrap().get(&id) {

...
fn make_entities(
    ...
    let inner_filestore = filestore.clone();
    ...
    (local_path, filestore, local, remote)
}
...
pub(crate) struct LossyTransport {
    pub(crate) socket: TokioUdpSocket,
...
impl TryFrom<(UdpSocket, HashMap<VariableID, SocketAddr>, TransportIssue)> for LossyTransport {
    ...
    fn try_from(
        ...
        Ok(Self {
            socket: TokioUdpSocket::from_std(inputs.0)?,
            entity_map: inputs.1,
            counter: 1,
            issue: inputs.2,
            buffer: vec![],
